...
def glyph(ws_number):
    ...
    glyphs = [" SHELL", " WWW", " CODE", "GAME", "TERM", "PIC", "TXT", "CODE"]
    glyphs = [" ", " ", " ", "4", "5", "6", "7", "8"]
    try:
...
def on_window_focus(i3, e):
    ...
    gl = glyph(con.workspace().num)
    if 'urxvt' in con.name.lower():
        gl = shell
    elif 'firefox' in con.name.lower() or 'chrom' in con.name.lower():
        gl = web
    ws_name = "%s: %s\u00a0%s" % (con.workspace().num, gl, con.name)
    #ws_name = "%s: %s\u00a0%s" % (con.workspace().num, glyph(con.workspace().num), n)
